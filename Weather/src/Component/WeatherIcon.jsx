  ...
  const [weatherType] =
    Object.entries(weatherTypes).find(
      ([weatherType, weatherCodes]) =>
        weatherCodes.includes(Number(weatherCode)) //indclude 可以判值是否存在陣列中
      //find 返回ture的[type,arrs]
    ) || [];
  return weatherType;
};
const WeatherIcon = ({ moment, currentWeatherCode }) => {
  ...
  return (
    <IconContainer>
      {
        weatherIcons[moment][currentWeatherIcon] //取物件值因為.無法接字串
      }
    </IconContainer>
  );
};
